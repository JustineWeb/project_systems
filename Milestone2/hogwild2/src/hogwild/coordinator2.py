...
def calculate_svm_update(id,w, data_train,targets_train,epochs,data_val,targets_val):
	...
	while n_epochs < epochs:
		...
		print(' Worker {} Val loss: {:.4f}'.format(id,val_loss))

		# check for early stopping
		stopping_crit_reached = early_stopping.stopping_criterion(val_loss)
		if stopping_crit_reached:
			break
	log = {'losses_val': losses_val}
	name = "logs" + str(id) + "_val.json" 
...
if __name__ == '__main__':
	...
	workers = [multiprocessing.Process(target=calculate_svm_update, args = (i,w,data_train,targets_train,s.epochs/s.N_WORKERS,data_val,targets_val)) for i in range(s.N_WORKERS)]
	...
	print("final time is: {}".format(end_time - start_time))
	print("time per iteration: {}".format((end_time - start_time)/s.epochs))
